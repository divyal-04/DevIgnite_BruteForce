...
def chat(request, username):
    ...
    # Add logic to refresh the page only once when the user accesses the chat endpoint
    refresh_once = request.session.get('refresh_once', True)
    if refresh_once:
        request.session['refresh_once'] = False
        return render(
            request,
            "chat.html",
            {
                ...
                "curr_user": usersen,
                "friend": friend,
                "refresh_once": refresh_once,  # Pass the flag to the template
            },
        )
    else:
        return render(
...
def SignupPage(request):
    ...
    if request.method == "POST":
        ...
        if User.objects.filter(username=uname).exists():
            ...
            return redirect("login")  # Change the redirect to login page
...
def LoginPage(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render(request, "login.html", {"error_message": error_message, "email": email})
        ...
        try:
            ...
            if verification_status.is_verified:
                ...
                error_message = "User not verified. Please check your email."
...
from  projects.models import Project  # Assuming your Project model is imported from the same app
from projects.models import  StarredProject
from profiles.models import Profile

@login_required
def dashboard(request):
    try:
        my_projects = Project.objects.filter(user=request.user)  # Fetch projects for the logged-in user
        all_projects = Project.objects.all() # Fetch all projects except those of the logged-in user
        starred_projects = StarredProject.objects.filter(user=request.user)  # Fetch starred projects for the logged-in user
        
        # Retrieve the profile for the logged-in user
        profile = Profile.objects.get(user=request.user)
        
        return render(request, 'dashboard.html', {'my_projects': my_projects, 'all_projects': all_projects, 'starred_projects': starred_projects, 'profile': profile})
...
def leaderboard(request):
    ...
    leaderboard = User.objects.annotate(total_stars=Sum('project__stars')).order_by('-total_stars')
    return render(request, 'statistics.html', {'leaderboard': leaderboard})
...
def send_verification_email(user_email):
    ...
    message = (
        f'Please click the link below to verify your email:<br><br>'
        f'<a href="{settings.BASE_URL}/app1/verify-email/?email={user_email}">'
        f'{settings.BASE_URL}/app1/verify-email/?email={user_email}</a>'
    )
    send_mail(
        subject,
        '',
...
def verify_email(request):
    ...
    try:
        ...
        try:
            ...
            VerificationStatus.objects.create(user=user, is_verified=True)
        
        # You may want to redirect the user to a page confirming their email verification
